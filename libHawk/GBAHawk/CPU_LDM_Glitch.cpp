...
namespace GBAHawk
{
			...
			switch ((cpu_Instr_ARM_2 >> 25) & 7)
			{
				case 0:
					...
					else
					{
						...
						cpu_Execute_Internal_Only_ARM_LDM();
					}
					break;
	...
	void GBA_System::cpu_Mul_Swap_Decode_ARM_LDM()
	{
		int64_t s1 = 0;
		int64_t s2 = 0;
		int64_t sf1 = 0;
	...
	void GBA_System::cpu_Execute_Internal_Only_ARM_LDM()
	{
		// local variables not stated (evaluated each time)
		uint32_t byte_mask = 0;
		uint32_t total_mask = 0;
