...
namespace GBAHawk
{
	...
	#pragma region PPU
		...
		uint16_t ppu_ROT_BG_X[2 * 240] = { };
		uint16_t ppu_ROT_BG_Y[2 * 240] = { };
		...
		void ppu_Write_Reg_8(uint32_t addr, uint8_t value)
		{
			switch (addr)
			{
				...
				case 0x20: ppu_BG_Rot_A[2] = (uint16_t)((ppu_BG_Rot_A[2] & 0xFF00) | value); break;
				case 0x21: ppu_BG_Rot_A[2] = (uint16_t)((ppu_BG_Rot_A[2] & 0x00FF) | (value << 8)); break;
				...
				case 0x24: ppu_BG_Rot_C[2] = (uint16_t)((ppu_BG_Rot_C[2] & 0xFF00) | value); break;
				case 0x25: ppu_BG_Rot_C[2] = (uint16_t)((ppu_BG_Rot_C[2] & 0x00FF) | (value << 8)); break;
				...
				case 0x30: ppu_BG_Rot_A[3] = (uint16_t)((ppu_BG_Rot_A[3] & 0xFF00) | value); break;
				case 0x31: ppu_BG_Rot_A[3] = (uint16_t)((ppu_BG_Rot_A[3] & 0x00FF) | (value << 8)); break;
				...
				case 0x34: ppu_BG_Rot_C[3] = (uint16_t)((ppu_BG_Rot_C[3] & 0xFF00) | value); break;
				case 0x35: ppu_BG_Rot_C[3] = (uint16_t)((ppu_BG_Rot_C[3] & 0x00FF) | (value << 8)); break;
		...
		void ppu_Write_Reg_32(uint32_t addr, uint32_t value)
		{
			switch (addr)
			{
				...
				case 0x20: ppu_BG_Rot_A[2] = (uint16_t)(value & 0xFFFF);
						   ppu_BG_Rot_B[2] = (uint16_t)((value >> 16) & 0xFFFF); break;
				case 0x24: ppu_BG_Rot_C[2] = (uint16_t)(value & 0xFFFF);
						   ppu_BG_Rot_D[2] = (uint16_t)((value >> 16) & 0xFFFF); break;
				...
				case 0x30: ppu_BG_Rot_A[3] = (uint16_t)(value & 0xFFFF);
						   ppu_BG_Rot_B[3] = (uint16_t)((value >> 16) & 0xFFFF); break;
				case 0x34: ppu_BG_Rot_C[3] = (uint16_t)(value & 0xFFFF);
						   ppu_BG_Rot_D[3] = (uint16_t)((value >> 16) & 0xFFFF); break;
		...
		void ppu_Render()
		{
			uint32_t bg_pixel_f = ppu_Transparent_Color;
			uint32_t bg_pixel_s = ppu_Transparent_Color;
			uint32_t spr_pixel = 0xFFFFFFFF;
			uint32_t final_pixel = 0xFFFFFFFF;

			uint32_t bld_pixel_1 = 0xFFFFFFFF;
			uint32_t bld_pixel_2 = 0xFFFFFFFF;
					...
					if (BG_Go[c0])
					{
						...
						if ((ppu_X_RS >= 0) && (ppu_Y_RS >= 0) && (ppu_X_RS < BG_Scale_X[c0]) && (ppu_Y_RS < BG_Scale_Y[c0]))
						{
							...
							else
							{
								...
								Pixel_Color += 16 * (BG_Effect_Byte >> 4);

								BG_Is_Transparent[c0] = (Pixel_Color & 0xF) == 0;
							}

							Pixel_Color_data = PALRAM_16[Pixel_Color];

							bg_pixel[c0] = (uint32_t)(0xFF000000 |
			...
			case 1:
					...
					if (BG_Go[c1])
					{
						...
						if ((ppu_X_RS >= 0) && (ppu_Y_RS >= 0) && (ppu_X_RS < BG_Scale_X[c1]) && (ppu_Y_RS < BG_Scale_Y[c1]))
						{
							...
							else
							{
								...
								BG_Is_Transparent[c1] = (Pixel_Color & 0xF) == 0;
							}

							Pixel_Color_data = PALRAM_16[Pixel_Color];

							bg_pixel[c1] = (uint32_t)(0xFF000000 |
								((Pixel_Color_data & 0x1F) << 19) |
								((Pixel_Color_data & 0x3E0) << 6) |
								((Pixel_Color_data & 0x7C00) >> 7));
				...
				if (BG_Go[2])
				{
					...
					if ((ppu_X_RS >= 0) && (ppu_Y_RS >= 0) && (ppu_X_RS < BG_Scale_X[2]) && (ppu_Y_RS < BG_Scale_Y[2]))
					{
						...
						bg_pixel[2] = (uint32_t)(0xFF000000 |
							((Pixel_Color_data & 0x1F) << 19) |
							((Pixel_Color_data & 0x3E0) << 6) |
							((Pixel_Color_data & 0x7C00) >> 7));
				...
				for (int c2 = 2; c2 < 4; c2++)
				{
					if (BG_Go[c2])
					{
						...
						if ((ppu_X_RS >= 0) && (ppu_Y_RS >= 0) && (ppu_X_RS < BG_Scale_X[c2]) && (ppu_Y_RS < BG_Scale_Y[c2]))
						{
							...
							bg_pixel[c2] = (uint32_t)(0xFF000000 |
								((Pixel_Color_data & 0x1F) << 19) |
								((Pixel_Color_data & 0x3E0) << 6) |
								((Pixel_Color_data & 0x7C00) >> 7));
			...
			case 3:
				...
				if (BG_Go[2])
				{
					...
					if ((ppu_X_RS < 240) && (ppu_Y_RS < 160) && (ppu_X_RS >= 0) && (ppu_Y_RS >= 0))
					{
						...
						Pixel_Color_data = VRAM_16[m3_ofst];

						bg_pixel_f = (uint32_t)(0xFF000000 |
									((Pixel_Color_data & 0x1F) << 19) |
									((Pixel_Color_data & 0x3E0) << 6) |
									((Pixel_Color_data & 0x7C00) >> 7));
			...
			case 5:
				...
				if (BG_Go[2])
				{
					...
					if ((ppu_X_RS < 160) && (ppu_Y_RS < 128) && (ppu_X_RS >= 0) && (ppu_Y_RS >= 0))
					{
						...
						Pixel_Color_data = VRAM_16[ppu_Display_Frame * 0x5000 + m3_ofst];
			...
			if (Brighten_Final_Pixel)
			{
				...
				final_pixel = (uint32_t)(0xFF000000 |
										(R << 3) |
										(G << 11) |
										(B << 19));
			}
										...
										(((ppu_Sprite_Attr_2 >> 10) & 3) < ppu_Sprite_Priority[ppu_Sprite_ofst_eval + cur_x_pos]))
									{
										...
										if ((actual_x_index < ppu_Sprite_X_Size) && (actual_y_index < ppu_Sprite_Y_Size))
										{
											...
											if ((pix_color & pal_scale) != 0)
											{
												if (spr_mode < 2)
												{
													// palette accesses are always 16 bit aligned
													pix_color = (uint32_t)(PALRAM_16[(0x200 + pix_color) >> 1]);

													pix_color = (uint32_t)(0xFF000000 |
														((pix_color & 0x1F) << 19) |
														((pix_color & 0x3E0) << 6) |
														((pix_color & 0x7C00) >> 7));
		...
		void ppu_Calculate_BG_Rotations(int layer)
		{
			int base_ofst = 0;
			...
			uint32_t Ref_X, Ref_Y;

			int32_t i_ref_x, i_ref_y;
			...
			if (ppu_BG_Mosaic[layer])
			{
				...
				for (int j = 0; j < 240; j++)
				{
					...
					sol_x = floor(sol_x + f_ref_x);
					sol_y = floor(-(sol_y - f_ref_y));

					ppu_ROT_BG_X[base_ofst + j] = (uint16_t)(sol_x);
					ppu_ROT_BG_Y[base_ofst + j] = (uint16_t)(sol_y);
				}
			}
			else
			{
		...
		void ppu_Calculate_Access_Timing()
		{
