...
namespace GBHawk
{
			...
			void WriteReg(int addr, uint8_t value)
			{
				switch (addr)
				{
					...
					case 0xFF41: // STAT
						...
						if (((STAT & 3) == 0) && STAT.Bit(3) && !glitch_state) { HBL_INT = true; }
	 else { HBL_INT = false; }
				...
				if (!HDMA_mode)
				{
					...
					else
					{
						...
						if ((HDMA_tick % 2) == 0)
						{
							...
							{
								HDMA_uint8_t = Core.ReadMemory(Core.cpu.RegPC);
								HDMA_VRAM_access_glitch--;
							}
							else
							{
								HDMA_uint8_t = Core.ReadMemory(cur_DMA_src);
							}
						}
						else
						{
	...
	void process_sprite()
	{
		...
		if (SL_sprites[sl_use_index * 4 + 3].Bit(5))
		{
			...
			sprite_sel[0] = (byte)(((temp_0 & 0x01) << 7) | ((temp_0 & 0x02) << 5) |
									((temp_0 & 0x04) << 3) | ((temp_0 & 0x08) << 1) |
									((temp_0 & 0x10) >> 1) | ((temp_0 & 0x20) >> 3) |
									((temp_0 & 0x40) >> 5) | ((temp_0 & 0x80) >> 7));

			sprite_sel[1] = (byte)(((temp_1 & 0x01) << 7) | ((temp_1 & 0x02) << 5) |
