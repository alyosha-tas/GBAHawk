...
namespace GBHawk
{
	...
	public:
		...
		void WriteReg(uint16_t addr, uint8_t value)
		{
			switch (addr)
			{
				case 0xFF40: // LCDC
					...
					if (LCDC_Bit(7) && ((value & 0x80) == 0x80) && LCDC_Bit(4) && !((value & 0x10) == 0x10))
					{
				...
				case 0xFF41: // STAT
					...
					if (((STAT & 3) == 0) && STAT_Bit(3) && !glitch_state) { HBL_INT = true; }
					else { HBL_INT = false; }

					if (((value & 0x40) == 0x40) && LCDC_Bit(7))
					{
						if (LY == LYC) { LYC_INT = true; }
						else { LYC_INT = false; }
					}
					if (!STAT_Bit(6)) { LYC_INT = false; }
					break;
				case 0xFF42: // SCY
					scroll_y = value;
					break;
				...
				case 0xFF4A: // WY
					...
					if (!window_started && (!LCDC_Bit(7) || (value > LY)))
					{
					...
					if (!HDMA_mode)
					{
						...
						else
						{
							...
							else
							{
								...
								{
									cur_DMA_dest = (uint16_t)((cur_DMA_dest + 1) & 0xFFFF);
									cur_DMA_src = (uint16_t)((cur_DMA_src + 1) & 0xFFFF);
						...
						if (HBL_HDMA_go && (HBL_HDMA_count > 0))
						{
							...
							else
							{
								...
								if ((HBL_HDMA_count == 0) && (HDMA_length != 0))
								{
									...
									HBL_HDMA_count = 0x10;
									HBL_HDMA_go = false;
									HDMA_countdown = *Core_Double_Speed ? 2 : 4;
								}
			...
			if (LCDC_Bit(7))
			{
				...
				if (cycle == 456)
				{
					...
					if (LY == 0 && LY_inc == 0)
					{
						...
						if (!LCDC_Bit(5)) { window_is_reset = true; }
					}
				...
				if (In_Vblank)
				{
					...
					if (LY == 144)
					{
						if (cycle <= 4)
						{
							if (!STAT_Bit(5)) { VBL_INT = false; }
						}

						if ((cycle >= 2) && (cycle < 4))
						{
							// there is an edge case where a VBL INT is triggered if STAT bit 5 is set
							if (STAT_Bit(5)) { VBL_INT = true; }
						}

						if (cycle >= 4)
						{
							if (STAT_Bit(4)) { VBL_INT = true; }
							else { VBL_INT = false; }
						}
					}
					else
						...
						if (cycle < 83)
						{
							if (cycle <= 4)
							{
								if ((cycle == 2) && (LY != 0))
								{
									...
									if (STAT_Bit(5)) { OAM_INT = true; }
								}

								// the last few cycles of mode 1 still trigger mode 1 int
								if (cycle < 4)
								{
									if (STAT_Bit(4) && ((STAT & 3) == 1)) { VBL_INT = true; }
									else { VBL_INT = false; }
								}
								else
								{
		...
		void render(uint16_t render_cycle)
		{
			...
			if (!fetch_sprite)
			{
				switch (read_case)
				{
					case 0: // read a background tile
						if ((internal_cycle % 2) == 1)
						{
							...
							BG_V_flip = Bit_Test(tile_data[2], 6);
					...
					case 8: // done reading, we are now in phase 0
						...
						if (*Core_Double_Speed) { HDMA_can_start = true; }
				...
				if (!was_pre_render)
				{
					...
					{
						if (Bit_Test(tile_data_latch[2], 5))
						{
						...
						if (!no_sprites)
						{
							...
							if (have_sprite)
							{
								if (LCDC_Bit(1))
								{
									...
									if (LCDC_Bit(0) && Bit_Test(tile_data_latch[2], 7) && (ref_pixel != 0))
									{
										use_sprite = false;
									}
								}
		...
		{
			int y;
			int VRAM_temp = (Bit_Test(SL_sprites[sl_use_index * 4 + 3], 3)) ? 1 : 0;
