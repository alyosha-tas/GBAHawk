...
namespace GBHawk
{
	...
	public:
		...
		{
			//Console.WriteLine((addr - 0xFF40) + " " + value + " " + LY + " " + cycle + " " + LCDC_Bit(7));
			switch (addr)
			{
				case 0xFF40: // LCDC
				...
				case 0xFF45:  // LYC
					...
					break;
				case 0xFF46: // DMA (hndled in core)
					break;
				case 0xFF47: // BGP
					BGP = value;
					break;
		...
		void render(uint16_t render_cycle)
		{
			...
			if (!no_sprites && !pre_render_2 && (pixel_counter < 160) && LCDC_Bit(1))
			{
				...
				switch (read_case)
				{
					case 0: // read a background tile
						if ((internal_cycle % 2) == 1)
						{
							...
							tile_byte  = Core_VRAM[bus_address];
					...
					case 4: // read from window data
						if ((window_counter % 2) == 1)
						{
							...
							bus_address = 0x1800 + (LCDC_Bit(6) ? 1 : 0) * 0x400 + temp_fetch;
							tile_byte = Core_VRAM[bus_address];
		...
		void process_sprite()
		{
			...
			if ((SL_sprites[sl_use_index * 4 + 3] & 0x40) == 0x40)
			{
				if (LCDC_Bit(2))
				{
					...
					y = 15 - y;
					sprite_sel[0] = Core_VRAM[(SL_sprites[sl_use_index * 4 + 2] & 0xFE) * 16 + y * 2];
					sprite_sel[1] = Core_VRAM[(SL_sprites[sl_use_index * 4 + 2] & 0xFE) * 16 + y * 2 + 1];
				}
				else
				{
					y = LY - (SL_sprites[sl_use_index * 4] - 16);
					y = 7 - y;
					sprite_sel[0] = Core_VRAM[SL_sprites[sl_use_index * 4 + 2] * 16 + y * 2];
					sprite_sel[1] = Core_VRAM[SL_sprites[sl_use_index * 4 + 2] * 16 + y * 2 + 1];
				}
			}
			else
			{
