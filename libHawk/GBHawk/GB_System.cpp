	...
	{			
		...
		switch (cpu_Instr_Type)
		{
			...
			case cpu_Load_Store_Word_ARM:
				...
				// Can be interrupted by an abort, but those don't occur in GB
				if (cpu_Fetch_Cnt == 0)
				{
			...
			case cpu_Internal_Can_Save_ARM:
				...
				// but it appears that the GB does not do so
				cpu_IRQ_Input_Use = cpu_IRQ_Input;
				...
				{
					// A memory access cycle could be saved here, but the GB does not seem to implement it
