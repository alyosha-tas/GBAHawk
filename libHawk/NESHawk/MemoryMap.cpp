...
namespace NESHawk
{
	...
	uint8_t NES_System::ReadMemory(uint32_t addr)
	{		
		uint8_t ret;
	...
	uint8_t NES_System::ReadMemoryDMA(uint32_t addr)
	{
		uint8_t ret;
		uint8_t ret2;
	...
	uint8_t NES_System::DummyReadMemory(uint32_t addr)
	{
		return ReadMemory(addr);
	}

	void NES_System::OnExecFetch(uint16_t addr)
	{

	}

	void NES_System::WriteMemory(uint32_t addr, uint8_t value)
	{
		address_bus = addr;
	...
	void NES_System::WriteReg(uint32_t addr, uint8_t val)
	{
		switch (addr)
		{
			case 0x4000:
	...
	uint8_t NES_System::peek_joyport(uint32_t addr)
	{
	...
	uint8_t NES_System::PeekMemory(uint32_t addr)
	{
		uint8_t ret;
	...
	uint8_t NES_System::PeekReg(uint32_t addr)
	{
		switch (addr)
		{
			case 0x4000:
	...
	uint16_t NES_System::Peek_Memory_16(uint32_t addr)
	{
		uint16_t ret = 0;
	...
	uint16_t NES_System::Peek_Memory_8_Branch(uint32_t addr)
	{
		int16_t ret = 0;
