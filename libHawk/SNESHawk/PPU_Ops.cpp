...
namespace SNESHawk
{
	...
	void SNES_System::write_2007(uint8_t value)
	{
		int addr = ppu_VRAM_Address & 0x3FFF;
		...
		if ((addr & 0x3F00) == 0x3F00)
		{
			// handle palette. this is being done SNEStopia style, because i found some documentation for it (appendix 1)
			addr &= 0x1F;
	...
	void SNES_System::write_2007_Glitch(uint8_t value)
	{
		int addr = ppu_VRAM_Address & 0x3FFF;
		if (ppuphase == PPU_PHASE_BG)
		{
