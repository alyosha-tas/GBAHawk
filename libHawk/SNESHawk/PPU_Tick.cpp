...
namespace SNESHawk
{
	void SNES_System::TickPPU(bool do_single_tick)
	{
			...
			if ((status_sl < 240) || (status_sl == 261))
			{
				...
				else if (status_cycle <= 256)
				{
					if (status_sl != 261)
					{
						...
						//at one point I commented this out to fix bottom-left garbage in DW4. but it's needed for full_SNES_palette. 
				...
				else if (status_cycle <= 320)
				{
					...
					switch (ppu_aux_index)
					{
						...
						case 2:
							...
							//tricky: tall sprites have already had liSNES>8 taken care of by getting a new pattern number above.
