...
// load IPL into the core
SNESHawk_EXPORT void SNES_load_ipl(SNESCore* p, uint8_t* ipl)
{
	p->Load_IPL(ipl);
}
...
SNESHawk_EXPORT uint8_t SNES_get_apu_ram(SNESCore* p, uint32_t addr) {
	return p->GetAPURAM(addr);
}
