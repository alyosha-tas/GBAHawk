...
namespace BizHawk.Emulation.Cores.Nintendo.GBA.Common
{
	...
	{
		public static void Setup_Mapper(string romHashMD5, string romHashSHA1, byte[] ROM, out int mapper, out bool has_bat, out uint Cart_RAM_Size)
		{
			int size_f = 0;
	...
	public class GBA_ControllerDeck
	{
		...
		public static IReadOnlyDictionary<string, Func<int, IPort>> ControllerCtors => _controllerCtors
			??= new Dictionary<string, Func<int, IPort>>
			{
				...
				[typeof(DiscreteSolar).DisplayName()] = portNum => new DiscreteSolar(portNum),
	...
	[DisplayName("Gameboy Advance Controller + Discrete Solar")]
	public class DiscreteSolar : IPort
	{
		public DiscreteSolar(int portNum)
		{
			PortNum = portNum;
			Definition = new ControllerDefinition("Gameboy Advance Controller + Discrete Solar")
			{
				BoolButtons = BaseDefinition.Select(b => $"P{PortNum} {b}").ToList()
			};
		}
