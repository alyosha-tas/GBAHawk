...
namespace BizHawk.Emulation.Cores.Nintendo.SNES.Common
{
	...
	public class SNES_ControllerDeck
	{
		public SNES_ControllerDeck(string controller1Name, string controller2Name, bool is_subrame = false)
		{
		...
		public static IReadOnlyDictionary<string, Func<int, IPort>> ControllerCtors => _controllerCtors
			??= new Dictionary<string, Func<int, IPort>>
			{
				...
				[typeof(UnpluggedSNES).DisplayName()] = portNum => new UnpluggedSNES(portNum)
			};

		public static string DefaultControllerName => typeof(SNESController).DisplayName();
	}
	...
	public class UnpluggedSNES : IPort
	{
		private bool _resetting;
		private int _latchedValue;

		public UnpluggedSNES(int portNum)
		{
			PortNum = portNum;
