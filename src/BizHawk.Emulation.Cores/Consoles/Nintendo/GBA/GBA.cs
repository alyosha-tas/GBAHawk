...
{
	[Core(CoreNames.GBA, "", isReleased: true)]
	[ServiceNotApplicable(new[] { typeof(IDriveLight) })]
	public partial class GBA : IEmulator, IVideoProvider, ISoundProvider, ISaveRam, IInputPollable, IRegionable, IGBAGPUViewable,
								ISettable<GBA.GBASettings, GBA.GBASyncSettings>
	{
		public byte[] BIOS;
		...
		public GBA(CoreComm comm, GameInfo game, byte[] rom, GBA.GBASettings settings, GBA.GBASyncSettings syncSettings)
		{
			...
			GBA_Pntr = LibGBA.GBA_create();

			LibGBA.GBA_load_bios(GBA_Pntr, BIOS);

			Console.WriteLine("Mapper: " + mppr);
			LibGBA.GBA_load(GBA_Pntr, ROM, (uint)ROM_Length, mppr);
			if (cart_RAM != null) { LibGBA.GBA_create_SRAM(GBA_Pntr, cart_RAM, (uint)cart_RAM.Length); }
			...
			Header_Length = LibGBA.GBA_getheaderlength(GBA_Pntr);
			Disasm_Length = LibGBA.GBA_getdisasmlength(GBA_Pntr);
			Reg_String_Length = LibGBA.GBA_getregstringlength(GBA_Pntr);

			var newHeader = new StringBuilder(Header_Length);
			LibGBA.GBA_getheader(GBA_Pntr, newHeader, Header_Length);
			...
			Mem_Domains.vram = LibGBA.GBA_get_ppu_pntrs(GBA_Pntr, 0);
			Mem_Domains.oam = LibGBA.GBA_get_ppu_pntrs(GBA_Pntr, 1);
			Mem_Domains.palram = LibGBA.GBA_get_ppu_pntrs(GBA_Pntr, 2);
			Mem_Domains.mmio = LibGBA.GBA_get_ppu_pntrs(GBA_Pntr, 3);

			GBA_message = null;

			LibGBA.GBA_setmessagecallback(GBA_Pntr, GBA_message);
		}
		...
		{
			LibGBA.GBA_Hard_Reset(GBA_Pntr);
		}
		...
		private LibGBA.TraceCallback tracecb;
		...
		private void MakeTrace(int t)
		{
			...
			LibGBA.GBA_getdisassembly(GBA_Pntr, new_d, t, Disasm_Length);
			LibGBA.GBA_getregisterstate(GBA_Pntr, new_r, t, Reg_String_Length);
		...
		private LibGBA.MessageCallback GBA_message;

		private void GetMessage()
		{
			StringBuilder new_m = new StringBuilder(200);

			LibGBA.GBA_getmessage(GBA_Pntr, new_m, 200);
