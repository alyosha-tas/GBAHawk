...
namespace BizHawk.Emulation.Cores.Nintendo.GBAHawk_Debug
{
	...
	{
		public void cpu_LDM_Glitch_Decode_ARM()
		{
		...
		public void cpu_LDM_Glitch_Execute_Internal_Only_ARM()
		{
		...
		public void cpu_LDM_Glitch_Tick()
		{
			switch (cpu_LDM_Glitch_Instr_Type)
			{
				...
				case cpu_Internal_Can_Save_ARM:
					...
					else
					{
						...
						if (cpu_LDM_Glitch_Mode)
						{
							if (cpu_IRQ_Input_Use & !cpu_FlagI) { cpu_Instr_Type = cpu_Prefetch_IRQ; cpu_LDM_Glitch_Mode = false; }
		...
		public uint cpu_LDM_Glitch_Get_Reg(uint reg_num)
		{
			uint ret = 0;
			...
			else
			{
				if (reg_num == 13)
				{
					ret = (uint)(cpu_Regs[reg_num] | cpu_user_R13);
				}
				else if (reg_num == 14)
				{
					ret = (uint)(cpu_Regs[reg_num] | cpu_user_R14);
				}
				else if ((cpu_Regs[16] & 0x1F) == 0x11)
				{
					if (reg_num == 8)
					{
						ret = (uint)(cpu_Regs[reg_num] | cpu_user_R8);
					}
					else if (reg_num == 9)
					{
						ret = (uint)(cpu_Regs[reg_num] | cpu_user_R9);
					}
					else if (reg_num == 10)
					{
						ret = (uint)(cpu_Regs[reg_num] | cpu_user_R10);
					}
					else if (reg_num == 11)
					{
						ret = (uint)(cpu_Regs[reg_num] | cpu_user_R11);
					}
					else if (reg_num == 12)
					{
						ret = (uint)(cpu_Regs[reg_num] | cpu_user_R12);
					}
				}
				else
				{
		...
		public void cpu_LDM_Glitch_Set_Reg(uint reg_num, uint value)
		{
