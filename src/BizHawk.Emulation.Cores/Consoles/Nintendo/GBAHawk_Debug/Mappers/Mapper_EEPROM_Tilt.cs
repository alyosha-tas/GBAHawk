...
{
	// Cart with EEEPROM and Tilt sensor
	public class MapperEEPROM_Tilt : MapperBase
	{
		...
		public bool ADC_Ready_X, ADC_Ready_Y;
		...
		// tilt sensor controls here
		public override byte ReadMemory8(uint addr)
		{
			if (addr == 0x8200)
			{
				return (byte)(Core.Acc_X_state);
			}
			else if (addr == 0x8300)
			{
				return (byte)(((Core.Acc_X_state >> 8) & 0xF) | (ADC_Ready_X ? 0x80 : 0));
			}
			else if (addr == 0x8400)
			{
				return (byte)(Core.Acc_Y_state);
			}
			else if (addr == 0x8500)
			{
				return (byte)(((Core.Acc_Y_state >> 8) & 0xF) | (ADC_Ready_Y ? 0x80 : 0));
			}
			else
			{
				return 0;
		...
		{
			// cannot access with 16 bit writes?
		}

		public override void WriteMemory32(uint addr, uint value)
		{
			// cannot access with 32 bit writes?
		}
		...
		public override void SyncState(Serializer ser)
		{
			...
			ser.Sync(nameof(ADC_Ready_X), ref ADC_Ready_X);
			ser.Sync(nameof(ADC_Ready_Y), ref ADC_Ready_Y);
		}
	}
}
