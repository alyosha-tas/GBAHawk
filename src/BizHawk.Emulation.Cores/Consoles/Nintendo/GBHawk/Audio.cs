...
namespace BizHawk.Emulation.Cores.Nintendo.GBHawk
{
	...
	public class Audio : ISoundProvider
	{
		...
		public static byte[] Wave_Init_GBC = { 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF,
	...
	public const int NR10 = 0;
		...
		// smooth out decay rate when removing a channel from the mixer
		public int Update_mixer_Counter;
		public int Mixer_SQ1_Output_L, Mixer_SQ1_Output_R;
		public int Mixer_SQ2_Output_L, Mixer_SQ2_Output_R;
		public int Mixer_Wave_Output_L, Mixer_Wave_Output_R;
		public int Mixer_Noise_Output_L, Mixer_Noise_Output_R;
			...
			if (AUD_CTRL_power)
			{
				switch (addr)
				{
					...
					case 0xFF25:                                        // NR51 (ctrl)
						...
						//Console.WriteLine("output at remove from mixer: " + WAVE_output);
					...
					case 0xFF3F:
						if (WAVE_enable)
						{
							Wave_RAM[WAVE_wave_cntr >> 1] = value;
						}
						else
						{
		...
		public void tick()
		{
			...
			if (sound_update_needed)
			{
				...
				if (AUD_CTRL_sq1_L_en) { L_final += SQ1_output; Mixer_SQ1_Output_L = SQ1_output; }
				else if (Mixer_SQ1_Output_L != 0)
				{
					L_final += Mixer_SQ1_Output_L;
					if (Update_mixer_Counter == 0) { Mixer_SQ1_Output_L -= 1; }
					sound_update_needed = true;
				}
				if (AUD_CTRL_sq2_L_en) { L_final += SQ2_output; Mixer_SQ2_Output_L = SQ2_output; }
				else if (Mixer_SQ2_Output_L != 0)
				{
					L_final += Mixer_SQ2_Output_L;
					if (Update_mixer_Counter == 0) { Mixer_SQ2_Output_L -= 1; }
					sound_update_needed = true;
				}
				if (AUD_CTRL_wave_L_en) { L_final += WAVE_output; Mixer_Wave_Output_L = WAVE_output; }
				else if (Mixer_Wave_Output_L != 0)
				{ 
					L_final += Mixer_Wave_Output_L;
					if (Update_mixer_Counter == 0) { Mixer_Wave_Output_L -= 1; }
					sound_update_needed = true;
				}
				if (AUD_CTRL_noise_L_en) { L_final += NOISE_output; Mixer_Noise_Output_L = NOISE_output; }
				else if (Mixer_Noise_Output_L != 0)
				{
					L_final += Mixer_Noise_Output_L;
					if (Update_mixer_Counter == 0) { Mixer_Noise_Output_L -= 1; }
					sound_update_needed = true;
				}

				if (AUD_CTRL_sq1_R_en) { R_final += SQ1_output; Mixer_SQ1_Output_R = SQ1_output; }
				else if (Mixer_SQ1_Output_R != 0)
				{
					R_final += Mixer_SQ1_Output_R;
					if (Update_mixer_Counter == 0) { Mixer_SQ1_Output_R -= 1; }
					sound_update_needed = true;
				}
				if (AUD_CTRL_sq2_R_en) { R_final += SQ2_output; Mixer_SQ2_Output_R = SQ2_output; }
				else if (Mixer_SQ2_Output_R != 0)
				{
					R_final += Mixer_SQ2_Output_R;
					if (Update_mixer_Counter == 0) { Mixer_SQ2_Output_R -= 1; }
					sound_update_needed = true;
				}
				if (AUD_CTRL_wave_R_en) { R_final += WAVE_output; Mixer_Wave_Output_R = WAVE_output; }
				else 
				{ 
					R_final += Mixer_Wave_Output_R;
					if (Mixer_Wave_Output_R != 0)
					{
						if (Update_mixer_Counter == 0) { Mixer_Wave_Output_R -= 1; }
						sound_update_needed = true;
					}
				}
				if (AUD_CTRL_noise_R_en) { R_final += NOISE_output; Mixer_Noise_Output_R = NOISE_output; }
				else if (Mixer_Noise_Output_R != 0)
				{
					R_final += Mixer_Noise_Output_R;
					if (Update_mixer_Counter == 0) { Mixer_Noise_Output_R -= 1; }
					sound_update_needed = true;
				}
			...
			Update_mixer_Counter += 1;
			Update_mixer_Counter &= 0xFF;
		}
		...
		public void Reset()
		{
			...
			Update_mixer_Counter = 0;
			Mixer_SQ1_Output_L = Mixer_SQ1_Output_R = 0;
			Mixer_SQ2_Output_L = Mixer_SQ2_Output_R = 0;
			Mixer_Wave_Output_L = Mixer_Wave_Output_R = 0;
			Mixer_Noise_Output_L = Mixer_Noise_Output_R = 0;
		...
		public void SyncState(Serializer ser)
		{
			...
			ser.Sync(nameof(Update_mixer_Counter), ref Update_mixer_Counter);

			ser.Sync(nameof(Mixer_SQ1_Output_L), ref Mixer_SQ1_Output_L);
			ser.Sync(nameof(Mixer_SQ1_Output_R), ref Mixer_SQ1_Output_R);
			ser.Sync(nameof(Mixer_SQ2_Output_L), ref Mixer_SQ2_Output_L);
			ser.Sync(nameof(Mixer_SQ2_Output_R), ref Mixer_SQ2_Output_R);
			ser.Sync(nameof(Mixer_Wave_Output_L), ref Mixer_Wave_Output_L);
			ser.Sync(nameof(Mixer_Wave_Output_R), ref Mixer_Wave_Output_R);
			ser.Sync(nameof(Mixer_Noise_Output_L), ref Mixer_Noise_Output_L);
			ser.Sync(nameof(Mixer_Noise_Output_R), ref Mixer_Noise_Output_R);
