...
namespace BizHawk.Emulation.Cores.Nintendo.NES
{
	public partial class MOS6502X
	{
		...
		private void Fetch3()
		{
			...
			if (RDY)
			{
				opcode3 = Core.ReadMemory(PC++);
			}
		}

		private void PushPCH()
		{
			Core.WriteMemory((ushort)(S-- + 0x100), (byte)(PC >> 8));
		}

		private void PushPCL()
		{
			Core.WriteMemory((ushort)(S-- + 0x100), (byte)PC);
		}

		private void PushP_BRK()
		{
			FlagB = true;
			Core.WriteMemory((ushort)(S-- + 0x100), P);
			FlagI = true;
			ea = BRKVector;
		}
		...
		private void PushP_Reset()
		{
			...
			{
				ea = ResetVector;
				Core.ReadMemory((ushort)(S-- + 0x100));
				FlagI = true;
			}
		}
		...
		private void FetchPCLVector()
		{
			...
			if (RDY)
			{
				...
				alu_temp = Core.ReadMemory((ushort)ea);
			}
		}
		...
		private void FetchPCHVector()
		{
			...
			if (RDY)
			{
				alu_temp += Core.ReadMemory((ushort)(ea + 1)) << 8;
				PC = (ushort)alu_temp;
			}
		}
		...
		private void Imp_CLC()
		{
			...
			if (RDY)
			{
				Core.ReadMemory(PC); FlagC = false;
			}
		}
		...
		private void Imp_CLD()
		{
			...
			if (RDY)
			{
				Core.ReadMemory(PC); FlagD = false;
			}
		}
		...
		private void Imp_CLV()
		{
			...
			if (RDY)
			{
				Core.ReadMemory(PC); FlagV = false;
			}
		}

		private void Abs_WRITE_STA()
		{
			Core.WriteMemory((ushort)((opcode3 << 8) + opcode2), A);
		}

		private void Abs_WRITE_STX()
		{
			Core.WriteMemory((ushort)((opcode3 << 8) + opcode2), X);
		}

		private void Abs_WRITE_STY()
		{
			Core.WriteMemory((ushort)((opcode3 << 8) + opcode2), Y);
		}

		private void Abs_WRITE_SAX()
		{
			Core.WriteMemory((ushort)((opcode3 << 8) + opcode2), (byte)(X & A));

		}

		private void ZP_WRITE_STA()
		{
			Core.WriteMemory(opcode2, A);
		}

		private void ZP_WRITE_STY()
		{
			Core.WriteMemory(opcode2, Y);
		}

		private void ZP_WRITE_STX()
		{
			Core.WriteMemory(opcode2, X);
		}

		private void ZP_WRITE_SAX()
		{
			Core.WriteMemory(opcode2, (byte)(X & A));
		}
		...
		private void IndIdx_Stage4()
		{
			...
			{
				alu_temp = ea + Y;
				ea = (Core.ReadMemory((byte)(opcode2 + 1)) << 8)
					| ((alu_temp & 0xFF));
			}
		}
		...
		{
			Core.WriteMemory((ushort)ea, A);
		}

		private void IndIdx_WRITE_Stage6_SHA()
		{
			Core.WriteMemory((ushort)ea, (byte)(A & X & 7));
		}
		...
		{
			Core.WriteMemory((ushort)ea, (byte)alu_temp);
		...
		private void DecS()
		{
			...
			if (RDY)
			{
				Core.ReadMemory((ushort) (0x100 | --S));
			}
		}
		...
		private void JSR()
		{
			...
			if (RDY)
			{
				PC = (ushort)((Core.ReadMemory((ushort)(PC)) << 8) + opcode2);
			}
		}
		...
		private void PullP()
		{
			...
			if (RDY)
			{
				P = Core.ReadMemory((ushort)(S++ + 0x100));
		...
		private void PullPCL()
		{
			...
			{
				PC &= 0xFF00;
				PC |= Core.ReadMemory((ushort)(S++ + 0x100));
			}
		}
		...
		private void PullPCH_NoInc()
		{
			...
			{
				PC &= 0xFF;
				PC |= (ushort)(Core.ReadMemory((ushort)(S + 0x100)) << 8);
			}
		}
		...
		private void Abs_READ_LDA()
		{
			...
			if (RDY)
			{
				A = Core.ReadMemory((ushort)((opcode3 << 8) + opcode2));
				NZ_A();
			}
		}
		...
		private void Abs_READ_LDY()
		{
			...
			if (RDY)
			{
				Y = Core.ReadMemory((ushort)((opcode3 << 8) + opcode2));
				NZ_Y();
			}
		}
		...
		private void Abs_READ_LDX()
		{
			...
			if (RDY)
			{
				X = Core.ReadMemory((ushort)((opcode3 << 8) + opcode2));
				NZ_X();
			}
		}
		...
		private void Abs_READ_BIT()
		{
			...
			if (RDY)
			{
				alu_temp = Core.ReadMemory((ushort)((opcode3 << 8) + opcode2));
				_Bit();
			}
		}
		...
		private void ZpIdx_RMW_Stage4()
		{
			...
			if (RDY)
			{
				alu_temp = Core.ReadMemory(opcode2);
			}
		}

		private void ZpIdx_RMW_Stage6()
		{
			Core.WriteMemory(opcode2, (byte)alu_temp);
		}
		...
		private void ZP_READ_NOP()
		{
			...
			if (RDY)
			{
				Core.ReadMemory(opcode2); //just a dummy
			}
		}
		...
		private void IdxInd_Stage4()
		{
			...
			if (RDY)
			{
				ea = Core.ReadMemory((ushort)alu_temp);
			}
		}
		...
		private void IdxInd_Stage5()
		{
			...
			if (RDY)
			{
				ea += (Core.ReadMemory((byte)(alu_temp + 1)) << 8);
			}
		}
		...
		private void PushA()
		{
			Core.WriteMemory((ushort)(S-- + 0x100), A);
		}
		...
		private void PullA_NoInc()
		{
			...
			if (RDY)
			{
				A = Core.ReadMemory((ushort)(S + 0x100));
				NZ_A();
			}
		}
		...
		private void JMP_abs()
		{
			...
			if (RDY)
			{
				PC = (ushort)((Core.ReadMemory(PC) << 8) + opcode2);
			}
		}
		...
		private void AbsIdx_Stage4()
		{
			...
			if (RDY)
			{
				...
				else alu_temp = Core.ReadMemory((ushort)ea);
			}
		}
		...
		{
			S = (byte)(X & A);
			Core.WriteMemory((ushort)ea, (byte)(S & (opcode3+1)));
		}
		...
		private void AbsInd_JMP_Stage5()
		{
			...
			if (RDY)
			{
				...
				alu_temp += Core.ReadMemory((ushort)ea) << 8;
				PC = (ushort)alu_temp;
			}
		}
