...
namespace BizHawk.Emulation.Cores.Nintendo.NES
{
	public partial class PPU
	{
		...
		{
			public bool color_disable; //Color disable (0: normal color; 1: AND all palette entries with 110000, effectively producing a monochrome display)
			public bool show_bg_leftmost; //Show leftmost 8 pixels of background
			public bool show_obj_leftmost; //Show sprites in leftmost 8 pixels
			public bool show_bg; //Show background
			public bool show_obj; //Show sprites
			public bool intense_green; //Intensify greens (and darken other colors)
			public bool intense_blue; //Intensify blues (and darken other colors)
			public bool intense_red; //Intensify reds (and darken other colors)

			public int intensity_lsl_6; //an optimization..

			public byte Value
			{
				get => (byte)((color_disable ? 1 : 0) | ((show_bg_leftmost ? 1 : 0) << 1) | 
							 ((show_obj_leftmost ? 1 : 0) << 2) | ((show_bg ? 1 : 0) << 3) | 
							 ((show_obj ? 1 : 0) << 4) | ((intense_green ? 1 : 0) << 5) | 
							 ((intense_blue ? 1 : 0) << 6) | ((intense_red ? 1 : 0) << 7));
				set
				{
					color_disable = (value & 1) == 1;
					show_bg_leftmost = ((value >> 1) & 1) == 1;
					show_obj_leftmost = ((value >> 2) & 1) == 1;
					show_bg = ((value >> 3) & 1) == 1;
					show_obj = ((value >> 4) & 1) == 1;
					intense_blue = ((value >> 6) & 1) == 1;
					intense_red = ((value >> 7) & 1) == 1;
					intense_green = ((value >> 5) & 1) == 1;
		...
		public ulong double_2007_read; // emulates a hardware bug of back to back 2007 reads
		...
		public sealed class Reg_2000
		{
			...
			public bool vram_incr32; //(0: increment by 1, going across; 1: increment by 32, going down)
			public bool obj_pattern_hi; //Sprite pattern table address for 8x8 sprites (0: $0000; 1: $1000)
			public int obj_high_ofst;
			public bool bg_pattern_hi; //Background pattern table address (0: $0000; 1: $1000)
			public int bg_high_ofst;
			public bool obj_size_16; //Sprite size (0: 8x8 sprites; 1: 8x16 sprites)
			public bool ppu_layer; //PPU layer select (should always be 0 in the NES; some Nintendo arcade boards presumably had two PPUs)
			public bool vblank_nmi_gen; //Vertical blank NMI generation (0: off; 1: on)

			public byte Value
			{
				get => (byte)(_regs._h | (_regs._v << 1) | ((vram_incr32 ? 1 :0) << 2) | 
							 ((obj_pattern_hi ? 1 : 0) << 3) | ((bg_pattern_hi ? 1 : 0) << 4) |
							 ((obj_size_16 ? 1 : 0) << 5) | ((ppu_layer ? 1 : 0) << 6) | ((vblank_nmi_gen ? 1 : 0) << 7));
				set
				{
					_regs._h = value & 1;
					_regs._v = (value >> 1) & 1;
					vram_incr32 = ((value >> 2) & 1) == 1;
					obj_pattern_hi = ((value >> 3) & 1) == 1;
					obj_high_ofst = obj_pattern_hi ? (1 << 0xC) : 0;
					bg_pattern_hi = ((value >> 4) & 1) == 1;
					bg_high_ofst = bg_pattern_hi ? (1 << 0xC) : 0;
					obj_size_16 = ((value >> 5) & 1) == 1;
					ppu_layer = ((value >> 6) & 1) == 1;
					vblank_nmi_gen = ((value >> 7) & 1) == 1;
				}
			}
		}


		private bool Reg2002_objoverflow;  //Sprite overflow. The PPU can handle only eight sprites on one scanline and sets this bit if it starts drawing sprites.
		private bool Reg2002_objhit; //Sprite 0 overlap.  Set when a nonzero pixel of sprite 0 is drawn overlapping a nonzero background pixel.  Used for raster timing.
		public bool Reg2002_vblank_active;  //Vertical blank start (0: has not started; 1: has started)
		...
		{
			return (byte)(((Reg2002_vblank_active ? 1 : 0) << 7) | ((Reg2002_objhit ? 1 : 0) << 6) | 
						 ((Reg2002_objoverflow ? 1 : 0) << 5) | (ppu_open_bus & 0x1F));
		}
		...
		private void write_2007(byte value)
		{
			...
			ppur.increment2007(ppur.status.rendering && PPUON, reg_2000.vram_incr32);
