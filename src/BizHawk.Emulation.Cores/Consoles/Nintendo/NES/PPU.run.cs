...
namespace BizHawk.Emulation.Cores.Nintendo.NES
{
	public partial class PPU
	{
		...
		{
			return reg_2000.bg_high_ofst | (par << 0x4) | ppur.fv;
		}
		...
		{
			if (ppur.status.cycle == 0 && ppur.status.sl == 242)
			{
			...
			else if (ppur.status.cycle == (3 + NMI_offset) && ppur.status.sl == 242)
			{
			...
			{
				if (ppur.status.sl == 261)
				{
					Reg2002_vblank_clear_pending = true;
					idleSynch ^= true;
					Reg2002_objhit = Reg2002_objoverflow = false;
				}
			}
		...
		public void TickPPU_active()
		{
			if (ppur.status.cycle < 256)
			{
				if (ppur.status.cycle == 0)
				{
					...
					if ((ppur.status.sl == 0) && PPUON && (reg_2003 >= 8))
					{
						for (int i = 0; i < 8; i++)
						{
				...
				switch (ppu_aux_index)
				{
					case 0:
						...
						if (reg_2000.obj_size_16)
						{
							...
							patternAddress = (patternNumber << 4) | reg_2000.obj_high_ofst;
					...
					case 1:
						if (ppur.status.sl == 0 && ppur.status.cycle == 305 && PPUON)
						{
							...
							// value gets pushed onto the ppu address bus
					...
					case 7:
						...
						else
						{
							...
							{
								byte temp_0 = t_oam[s].patterns_0;
								byte temp_1 = t_oam[s].patterns_1;

								t_oam[s].patterns_0 = (byte)(((temp_0 & 0x01) << 7) | ((temp_0 & 0x02) << 5) |
															 ((temp_0 & 0x04) << 3) | ((temp_0 & 0x08) << 1) |
															 ((temp_0 & 0x10) >> 1) | ((temp_0 & 0x20) >> 3) |
															 ((temp_0 & 0x40) >> 5) | ((temp_0 & 0x80) >> 7));

								t_oam[s].patterns_1 = (byte)(((temp_1 & 0x01) << 7) | ((temp_1 & 0x02) << 5) |
			...
			else
			{
				...
				else
				{
					...
					if (ppur.status.sl == 0 && idleSynch && evenOddDestiny)
		...
		// One scanline of post render
		public void TickPPU_preVBL()
		{
			if (ppur.status.cycle == 1)
			{
				// value gets pushed onto the ppu address bus (fixes Recca)
				nes.Board.AddressPpu(ppur.get_2007access());

				// assume soam decays in between VBL and scanline zero.
				// this fixes Burai Fighter, which may get stray clocks otherwise on scanline zero
				// where sprite tiles are chosen from old soam data
				for (int i = 0; i < 64; i++) { soam[i] = 0xFF; }
			}
